...
class ShowerEnv(gym.Env):
    ...
    def step(self, action):
        ...
        elif self.nome_algoritmo in ["soft_actor_critic", "gpi-ls"]:
            if self.nome_algoritmo == "gpi-ls":
...
def treina_agente(nome_algoritmo, n_iter_agente, n_iter_checkpoints, Tinf_list, real_Tinf, custo_eletrico_kwh_list, only_iqb, total_timesteps=50000):
    ...
    if nome_algoritmo == "gpi-ls":
        ...
        model_path = os.path.join(path_root, f"gpi_ls_agent_BM3.zip" + only_iqb_flag)
        agent.save(model_path)
        print(f"Modelo GPILSContinuousAction salvo em: {model_path}")

    elif nome_algoritmo in ["proximal_policy_optimization", "soft_actor_critic"]:
...
def avalia_agente(nome_algoritmo, Tinf_list, real_Tinf, custo_eletrico_kwh_list, only_iqb):
    ...
    # Se quiser verificar uma temperatura em específico
    if real_Tinf:
        Tinf_list.clear()
        Tinf_list = [real_Tinf]
    ...
    if nome_algoritmo != "gpi-ls":
        ...
        checkpoint_path = path_root + "gpi_ls_agent_BM3.zip" + only_iqb_flag +"/gpi_ls_BM3.tar"
    ...
    ax[2].legend()
    plt.savefig(path_imagens + "resultado1_" + nome_algoritmo + "_Tinf" + "1" + "_tarifa" + custo_eletrico_kwh_var + ".png", dpi=200)
    plt.cla()
    plt.close(fig)
    ...
    ax[1, 1].legend()
    plt.savefig(path_imagens + "resultado2_" + nome_algoritmo + "_Tinf" + "1" + "_tarifa" + custo_eletrico_kwh_var + ".png", dpi=200)
    plt.cla()
    plt.close(fig)
    ...
    ax[2].legend()
    plt.savefig(path_imagens + "resultado3_" + nome_algoritmo + "_Tinf" + "1" + "_tarifa" + custo_eletrico_kwh_var + ".png", dpi=200)
    plt.cla()
    plt.close(fig)
    ...
    ax.legend()
    plt.savefig(path_imagens + "resultado4_" + nome_algoritmo + "_Tinf" + "1" + "_tarifa" + custo_eletrico_kwh_var + ".png", dpi=200)
    plt.cla()
    plt.close(fig)
