...
class ShowerEnv(gym.Env):
    ...
    def step(self, action):
        ...
        max_erro_temp = 10.0
        erro_temp = abs((self.Ts - 38.0))
        reward_temp = max(0.0, 1.0 - (erro_temp / max_erro_temp))

        max_vazao = 14.0
        reward_vazao = self.Fs / max_vazao

        reward = np.array([reward_temp,
                           reward_vazao], dtype=np.float32)
...
def avalia_agente(nome_algoritmo, Tinf):
    ...
    path_imagens = os.getcwd() + "/imagens_testeHiperparam_Tinf" + Tinf_var + "/"
